...
function analyzeSchema(schema, options = {}) {
    ...
    for (const column of table.columns) {
      ...
      if (shouldSuggestForeignKey(colName, tablePattern, mode)) {
        ...
        if (!hasFk && matchedTable) {
          // Check FK type compatibility with referenced PK
  ...
  for (const table of tables) {
    for (const column of table.columns) {
      ...
      // Recommend index on commonly filtered lookup/filter columns
  ...
  // 6b. Check for updated_at columns that won't auto-update
  for (const table of tables) {
  ...
  // 6c. Check for order_items-like tables missing composite unique constraint
  for (const table of tables) {
