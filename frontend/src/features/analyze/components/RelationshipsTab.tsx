...
import type { Relationship, Issue, Table } from '../../../shared/types/schema';
...
export default function RelationshipsTab({ relationships, issues, tables }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 10, padding: '18px 20px' }}>
        ...
        {relationships.length > 0 ? (
          ...
          <div style={{ padding: '8px 0', fontSize: 13, color: 'var(--text-muted)' }}>
            No foreign keys defined in this schema.
          </div>
        )}
      </div>
      ...
      {/* FK Issues */}
      <div style={{ background: 'var(--surface-2)', border: `1px solid ${fkIssues.length > 0 ? 'rgba(244,63,94,0.2)' : 'var(--border)'}`, borderRadius: 10, padding: '18px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, paddingBottom: 10, borderBottom: '1px solid var(--border-subtle)' }}>
          {fkIssues.length > 0 ? <AlertCircle size={13} color="#f43f5e" /> : <CheckCircle2 size={13} color="#10b981" />}
        ...
        {fkIssues.length > 0 ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0' }}>
            ...
            <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>No relationship issues found.</span>
          </div>
        )}
      </div>
    </div>
