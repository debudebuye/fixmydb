...
import type { NormalizationAnalysis } from '../../../shared/types/schema';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ height: 6, background: 'var(--border-subtle)', borderRadius: 99, overflow: 'hidden', marginBottom: 8 }}>
      ...
      {violations.length > 0 && (
        <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 10, padding: '18px 20px' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 12, paddingBottom: 10, borderBottom: '1px solid var(--border-subtle)' }}>
            Violation Details
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {violations.map((v, i) => (
              <div key={i} style={{ background: 'var(--surface-1)', border: '1px solid var(--border-subtle)', borderRadius: 8, padding: '12px 14px' }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6, flexWrap: 'wrap' }}>
                  ...
                  {v.column && <code style={{ fontSize: 11, color: 'var(--text-secondary)', fontFamily: 'monospace' }}>.{v.column}</code>}
                </div>
                <p style={{ fontSize: 12, color: '#f59e0b', fontWeight: 600, marginBottom: 4 }}>{v.violation}</p>
                <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8 }}>{v.explanation}</p>
                <div style={{ display: 'flex', gap: 6, fontSize: 12, color: '#10b981', padding: '8px 10px', background: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.15)', borderRadius: 6 }}>
                  ...
                  <span style={{ color: 'var(--text-muted)' }}>{v.suggestion}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {suggestions.length > 0 && (
        <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 10, padding: '18px 20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {suggestions.map((s, i) => (
              <div key={i} style={{ background: 'var(--surface-1)', border: '1px solid var(--border-subtle)', borderRadius: 8, padding: '10px 12px' }}>
                ...
                <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>{s.message}</p>
